        ...
        <div class="col-lg-12">
            <div class="ibox ">
                <div class="ibox-title">
                    <a class="btn btn-primary" href="{{route('estados.agregar')}}">Agregar</a>
                    <div class="ibox-tools"><a class="collapse-link"><i class="fa fa-chevron-up"></i></a></div>
                </div>
                <div class="ibox-content">
                    <form name="formBuscar" action="{{route("estados.index")}}" method="get">
                        <div class="row">
                        ...
                        <tbody>
                            @foreach($estados as $estado)
                                <tr>
                                    ...
                                    <td>{{fecha_latina($estado->created_at) }}</td>
                                    <td>{{fecha_latina($estado->updated_at) }}</td>
                                    <td data-texto="{{$estado->estado}}">
                                        <a href="{{route('estados.mostrar',$estado->id)}}" title="Mostrar"><img width="17px" src="{{asset('img/iconos/mostrar.png')}}" alt="Mostrar"></a>
                                        {{--<a href="{{route('estados.modificar',$estado->id)}}" title="Modificar"><img width="17px" src="{{asset('img/iconos/modificar.png')}}" alt="Modificar"></a>--}}
                                        <a data-ruta="{{route('estados.eliminar',$estado->id)}}" class="btn-eliminar" title="Eliminar"><img width="17px" src="{{asset('img/iconos/eliminar.png')}}" alt="Eliminar"></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
