        ...
        <div class="col-lg-12">
            <div class="ibox ">
                <div class="ibox-title">
                    <a class="btn btn-primary" href="{{route('formas_pagos.agregar')}}">Agregar</a>
                    <div class="ibox-tools"><a class="collapse-link"><i class="fa fa-chevron-up"></i></a></div>
                </div>
                <div class="ibox-content">
                    <form name="formBuscar" action="{{route("formas_pagos.index")}}" method="get">
                        <div class="row">
                        ...
                        <tbody>
                            @foreach($formas_pagos as $forma_pago)
                                <tr>
                                    ...
                                    <td>{{fecha_latina($forma_pago->created_at) }}</td>
                                    <td>{{fecha_latina($forma_pago->updated_at) }}</td>
                                    <td data-texto="{{$forma_pago->tipo}}">
                                        <a href="{{route('formas_pagos.mostrar',$forma_pago->id)}}" title="Mostrar"><img width="17px" src="{{asset('img/iconos/mostrar.png')}}" alt="Mostrar"></a>
                                        {{--<a href="{{route('formas_pagos.modificar',$forma_pago->id)}}" title="Modificar"><img width="17px" src="{{asset('img/iconos/modificar.png')}}" alt="Modificar"></a>--}}
                                        <a data-ruta="{{route('formas_pagos.eliminar',$forma_pago->id)}}" class="btn-eliminar" title="Eliminar"><img width="17px" src="{{asset('img/iconos/eliminar.png')}}" alt="Eliminar"></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
