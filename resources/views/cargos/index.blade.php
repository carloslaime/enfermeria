        ...
        <div class="col-lg-12">
            <div class="ibox ">
                <div class="ibox-title">
                    <a class="btn btn-primary" href="{{route('cargos.agregar')}}">Agregar</a>
                    <div class="ibox-tools"><a class="collapse-link"><i class="fa fa-chevron-up"></i></a></div>
                </div>
                <div class="ibox-content">
                    <form name="formBuscar" action="{{route("cargos.index")}}" method="get">
                        <div class="row">
                        ...
                        <tbody>
                            @foreach($cargos as $cargo)
                                <tr>
                                    ...
                                    <td>{{fecha_latina($cargo->created_at) }}</td>
                                    <td>{{fecha_latina($cargo->updated_at) }}</td>
                                    <td data-texto="{{$cargo->nombre}}">
                                        <a href="{{route('cargos.mostrar',$cargo->id)}}" title="Mostrar"><img width="17px" src="{{asset('img/iconos/mostrar.png')}}" alt="Mostrar"></a>
                                        <a href="{{route('cargos.modificar',$cargo->id)}}" title="Modificar"><img width="17px" src="{{asset('img/iconos/modificar.png')}}" alt="Modificar"></a>
                                        <a data-ruta="{{route('cargos.eliminar',$cargo->id)}}" class="btn-eliminar" title="Eliminar"><img width="17px" src="{{asset('img/iconos/eliminar.png')}}" alt="Eliminar"></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
