        ...
        <div class="col-lg-12">
            <div class="ibox ">
                <div class="ibox-title">
                    <a class="btn btn-primary" href="{{route('reservas.agregar')}}">Agregar</a>
                    <div class="ibox-tools"><a class="collapse-link"><i class="fa fa-chevron-up"></i></a></div>
                </div>
                <div class="ibox-content">
                    <form name="formBuscar" action="{{route("reservas.index")}}" method="get">
                        <div class="row">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @foreach($reservas as $reservas)
                                <tr>
                                    <td>{{$reservas->id}}</td>
                                    <td>{{$reservas->paciente}}</td>
                                    <td>{{$reservas->hora}}</td>
                                    <td>{{$reservas->fecha}}</td>
                                    <td>{{$reservas->servicio}}</td>
                                    <td>{{$reservas->total}}</td>
                                    <td>{{$reservas->formas_pagos}}</td>
                                    <td>
                                        @if ($reservas->activo) 
                                    ...
                                    <td>{{fecha_latina($reservas->created_at) }}</td>
                                    <td>{{fecha_latina($reservas->updated_at) }}</td>
                                    <td>
                                        <a href="{{route('reservas.mostrar',$reservas->id)}}" title="Mostrar"><img width="17px" src="{{asset('img/iconos/mostrar.png')}}" alt="Mostrar"></a>
                                        <a href="{{route('reservas.modificar',$reservas->id)}}" title="Modificar"><img width="17px" src="{{asset('img/iconos/modificar.png')}}" alt="Modificar"></a>
                                        <a data-ruta="{{route('reservas.eliminar',$reservas->id)}}" class="btn-eliminar" title="Eliminar"><img width="17px" src="{{asset('img/iconos/eliminar.png')}}" alt="Eliminar"></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
