        ...
        <div class="col-lg-12">
            <div class="ibox ">
                <div class="ibox-title">
                    <a class="btn btn-primary" href="{{route('uvs.agregar')}}">Agregar</a>
                    <div class="ibox-tools"><a class="collapse-link"><i class="fa fa-chevron-up"></i></a></div>
                </div>
                <div class="ibox-content">
                    <form name="formBuscar" action="{{route("uvs.index")}}" method="get">
                        <div class="row">
                        ...
                        <tbody>
                            @foreach($uvs as $uv)
                                <tr>
                                    ...
                                    <td>{{fecha_latina($uv->created_at) }}</td>
                                    <td>{{fecha_latina($uv->updated_at) }}</td>
                                    <td data-texto="{{$uv->numero}}">
                                        <a href="{{route('uvs.mostrar',$uv->id)}}" title="Mostrar"><img width="17px" src="{{asset('img/iconos/mostrar.png')}}" alt="Mostrar"></a>
                                        <a href="{{route('uvs.modificar',$uv->id)}}" title="Modificar"><img width="17px" src="{{asset('img/iconos/modificar.png')}}" alt="Modificar"></a>
                                        <a data-ruta="{{route('uvs.eliminar',$uv->id)}}" class="btn-eliminar" title="Eliminar"><img width="17px" src="{{asset('img/iconos/eliminar.png')}}" alt="Eliminar"></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
